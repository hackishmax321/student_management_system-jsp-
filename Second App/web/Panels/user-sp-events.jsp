...
<% 
  ...
  <div class="table-box">
    <table cellpadding="10">
      <tr><th>No.</th><th>Event Description</th></tr>
      <tr ><td>+</td><td id="spec-td" style="display: <%= display %>"><center><a href="<%=request.getContextPath()%>/Panels/event-uploadSP.jsp"><ion-icon name="add-circle" style="font-size: 70px"></ion-icon><h6>ADD NEW EVENT</h6></a></center></td></tr>
      <%
                                                                    ...
                                                                    int x = 0;
                                                                    String query = "Select COUNT(*) from university_spevent";
                                                                    ...
                                                                    for(int i=0;i<x;i++){
                                                                        String query2 = "Select university_spevent.*, identified_university.Name from university_spevent, identified_university where ID="+(i+1)+" AND identified_university.UniID = university_spevent.UniID";
                                                                        //stmt.setInt(1, (i+1));
                                                                        //ob2.println(stmt);
                                                                        ...
                                                                        while(rs.next()){
                                                                            %> 
      <tr><td><b><%=(i+1)%>.</b></td><td>
              <a href="spevent-page.jsp?g=<%=rs.getString(1)%>" style="text-decoration: none; "><h1 class="h1-level02"><%=rs.getString(2)%></h1></a>
                                <p id="spe">By <%=rs.getString(9)%></p><br>
                                <div class="im-box" style="background:url('<%= request.getContextPath()%>/<%=rs.getString(7)%>'); background-repeat: no-repeat;background-size: cover;background-position: center;"></div>
				<p><%=rs.getString(6)%> A humorous take on Sir Arthur Conan Doyle's classic mysteries featuring Sherlock Holmes and Doctor Watson.</p>
				<p>
...
"Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis et quasi architecto beatae vitae dicta sunt explicabo. Nemo enim ipsam voluptatem quia voluptas sit aspernatur aut odit aut fugit, sed quia consequuntur magni dolores eos qui ratione voluptatem sequi nesciunt. Neque porro quisquam est, qui dolorem ipsum quia dolor sit amet, consectetur, adipisci velit, sed quia non numquam eius modi tempora incidunt ut labore et dolore magnam aliquam quaerat voluptatem. Ut enim ad minima veniam, quis nostrum exercitationem ullam corporis suscipit laboriosam, nisi ut aliquid ex ea commodi consequatur? Quis autem vel eum iure reprehenderit qui in ea voluptate velit esse quam nihil molestiae consequatur, vel illum qui dolorem eum fugiat quo voluptas nulla pariatur?"</p>
				<br>
				<p id="spe">Starting Date	:<%=rs.getString(3)%></p><br>
                                <p id="spe">Ending Date         :<%=rs.getString(4)%></p><br>
				<p id="spe">Venue	:<%=rs.getString(5)%></p><br>
