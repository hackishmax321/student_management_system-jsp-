        ...
        <div class="row" style="padding: 17px;">
          ...
          <div class="card-body">
                  ...
                  <%
                      ...
                      for(int i=0; i<l; i++){
                          rs = st.executeQuery("Select * from private_message where Receiver = '"+session.getAttribute("username").toString()+"' AND ID = "+(l-i)+"");
                          if(rs.next()){
                               count++;
                              String ft = "#0652DD";
                              String b = "bolder";
                              ...
                              <div class="form-group" style="border-bottom: 2px solid #33ccff; border-left: 1px solid #33ccff; padding: 2px;">
                                     <div>
                                         <p style="color:<%=ft%>; font-weight:<%=b%>;"><%=count%>.    <%=rs.getString(2) %>   - <a href="profile_page.jsp?name=<%=rs.getString(4)%>" class="btn btn-outline-dark" ><%=rs.getString(4) %></a></p><div style="float: right; position: relative; top:-45px;"><a href="private-message.jsp?g=<%=session.getAttribute("username").toString()%>&r=<%=rs.getString(4) %>" class="btn btn-primary" name="btn-rep" >Reply</a><a href="DBjspAction/dbDeny.jsp?u=<%=rs.getString(1)%>" class="btn btn-danger" name="btn-sub" style="margin: 2px;" >Delete</a><a href="DBjspAction/dbAllocate.jsp?u=<%=rs.getString(1)%>&n=<%=session.getAttribute("username").toString()%>" class="btn btn-info" name="btn-sub" >Mark as Read</a></div>

                                     </div>
                                     
                                      
                                      
                                      <div id ="hide-panel" style="width: 80%; padding: 5px 20px;">
                                          <p style="color:<%=ft%>;"> <%=rs.getString(3) %> </p>
                                      </div>   
                              </div>
        ...
        <div class="row" style="padding: 17px;">
          ...
          <div class="card-body">
                  ...
                  <%
                      ...
                      for(int i=0; i<l; i++){
                          rs = st.executeQuery("Select * from private_message where Sender = '"+session.getAttribute("username").toString()+"' AND ID = "+(l-i)+"");
                          if(rs.next()){
                               count2++;
                              String ft = "#0652DD";
                              %>
                              <div class="form-group" style="border-bottom: 2px solid #FC427B; border-left: 1px solid #FC427B; padding: 2px;">
                                     <div>
                                         <p><%=count2%>.    <%=rs.getString(2) %>   - <a href="profile_page.jsp?name=<%=rs.getString(5)%>" class="btn btn-outline-dark" ><%=rs.getString(5) %></a></p><div style="float: right; position: relative; top:-45px;"><a href="DBjspAction/dbDeny.jsp?u=<%=rs.getString(1)%>" class="btn btn-danger" name="btn-sub" >Delete</a></div>
