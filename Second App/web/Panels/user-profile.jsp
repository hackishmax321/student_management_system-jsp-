...
<div class="user-box">
    ...
    <%
        if(session.getAttribute("user-role").toString()=="Undergraduate"){
            rs = st.executeQuery("Select UserImg from users2 where FirstName = '"+session.getAttribute("username").toString()+"' ");
        } else if((session.getAttribute("user-role").toString()=="Admin")||(session.getAttribute("user-role").toString()=="Staff")){
            rs = st.executeQuery("Select UserImg from university where Username = '"+session.getAttribute("username").toString()+"' ");
        } else if(session.getAttribute("user-role").toString()=="Industry"){
            rs = st.executeQuery("Select UserImg from industry where ChiefPerson = '"+session.getAttribute("username").toString()+"' ");
        } else if(session.getAttribute("user-role").toString()=="Super Admin") {
            rs = st.executeQuery("Select UserImg from site_admin where Username = '"+session.getAttribute("username").toString()+"' ");
        }
    ...
    <div class="user-img" style="background: url('data:image/jpeg;base64,${imgBase}'); background-size: cover;"><div id="im-edit"><a href="profileImg-box.jsp?i=<%=session.getAttribute("username").toString()%>&j=<%=session.getAttribute("user-role").toString()%>">EDIT</a></div></div>
    ...
    <div class="user-img" style="background: url('Images/user.png'); background-size: cover;"><div id="im-edit"><a href="profileImg-box.jsp?i=<%=session.getAttribute("username").toString()%>&j=<%=session.getAttribute("user-role").toString()%>">EDIT</a></div></div>    
        <%
        }
        %>
