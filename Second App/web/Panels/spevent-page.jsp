...
<% 
    ...
    rs = st.executeQuery("Select university_spevent.*, identified_university.Name from university_spevent, identified_university where ID="+i+" AND identified_university.UniID = university_spevent.UniID");
...
<div class="user-box">
  ...
  <% 
      while(rs.next()){
          %>
  <div id ="im-boxV2" style="background-image:url('<%= request.getContextPath()%>/<%=rs.getString(7)%>')"></div>
  <div class="details">
    <h2><%=rs.getString(2)%></h2>
    <p><b><%=rs.getString(9)%></b></p>
    <p><%=rs.getString(6)%></p>
    ...
    <div class="line">
     <label>Starting Date   :</label><label><%=rs.getString(3) %></label>
    </div>
    <div class="line">
     <label>Ending Date   :</label><label><%=rs.getString(4) %></label>
    </div>
    <div class="line">
